...
class MicrophonePowerObserver: ObservableObject{
    ...
    func startobserving(){
        do{
            let recorderStrings: [String: Any] = [
                AVFormatIDKey: NSNumber(value: kAudioFormatAppleLossless),
                AVNumberOfChannelsKey: 1
            ]
            
            let recorder = try AVAudioRecorder(url: URL(fileURLWithPath: "dev/null", isDirectory: true), settings: recorderStrings)
            ...
            self.cancellable = Timer.publish(
                every: 1.0/powerRatioEmissionsPerSecond, tolerance: 1.0/powerRatioEmissionsPerSecond, on: .main, in: .common
            )
            .autoconnect()
            .sink{
                [weak self] _ in
