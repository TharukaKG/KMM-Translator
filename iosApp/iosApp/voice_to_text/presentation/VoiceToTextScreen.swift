...
//  iosApp
//
//  Created by Tharuka Gamage on 2023-02-22.
...
struct VoiceToTextScreen: View {
    ...
    let parser: any VoiceToTextParser
    ...
    init(onResult: @escaping (String) -> Void, parser: any VoiceToTextParser, languageCode: String) {
        ...
        viewModel = IOSVoiceToTextViewModel(parser: parser, languageCode: languageCode)
    }
    ...
    var body: some View {
        VStack{
            ...
            HStack{
                Spacer()
                VoiceRecorderButton(displayState: viewModel.state.displayState ?? .waitingToTalk, onClick: {
                    if viewModel.state.displayState != .displayingResults{
                        viewModel.onEvent(event: VoiceToTextEvent.ToggleRecord(languageCode: languageCode))
                    }else{
    ...
    var mainView: some View{
        if let displayState = viewModel.state.displayState{
            ...
            case .error:
                return AnyView(
                    Text(
                        viewModel.state.recordError ?? "unknown Error"
                    )
                    .font(.title2)
                    .foregroundColor(.red)
                )
