        ...
        var body: some View{
            VStack(alignment: .leading){
                ...
                LanguageDisplay(language: toLanguage)
                    .padding(.bottom)
                Text(toText).foregroundColor(.onSurface)
                HStack{
                    Spacer()
                    Button(action:{
                        tts.speak(text: toText, language: fromLanguage.language.langCode)
                    }){
                        Image(systemName: "speaker.wave.2").renderingMode(.template).foregroundColor(.lightBlue)
                    }
                    Button(action:{
                        UIPasteboard.general.setValue(fromText, forPasteboardType: UTType.plainText.identifier)
                    }){
...
struct TranslateTextField_Previews: PreviewProvider {
    static var previews: some View {
        TranslateTextField(
            ...
            toText: "Hola",
            isTranslating: false,
            fromLanguage: UiLanguage(language: .german, imageName: "german"),
